...
<div class="col-12">
    ...
    <div class="row g-2 align-items-end mb-2" id="searchContainer">
        ...
        <div class="col-auto col-sm-8" style="width: 20%;">
            ...
            <select id="filterItem" class="form-select w-100" name="item[]" multiple>
                <?php foreach ($item_list as $k => $v)
                {
                    $selected = '';
                    if (isset($param_search['item']) && is_array($param_search['item']) && in_array($v->id, $param_search['item'])) {
                        $selected = 'selected';
                    }
                ?>
                    <option value="<?php echo $v->id; ?>" <?php echo $selected; ?>><?php echo $v->item_name; ?></option>
                <?php } ?>
            </select>
        </div>
        <div class="col-auto col-sm-8" style="width: 20%;">
...
<script>
    ...
    function formatOptionWithCheckbox(option) {
        ...
        let isSelected = $(option.element).prop('selected');

        return `
            <div class="d-flex align-items-center">
                <input type="checkbox" style="margin-right: 6px;" ${isSelected ? 'checked' : ''} disabled/>
                <span>${option.text}</span>
            </div>
        `;
    }

    function formatSelected(selection) {
        const selected = $('#filterItem').select2('data');
